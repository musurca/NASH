--[[
https://github.com/wyozi/lua-pngencoder
]]--
...
local function putBigUint32(val, tbl, index)
    for i=0,3 do
        tbl[index + i] = bit_and(bit_rshift(val, (3 - i) * 8), 0xFF)
    end
end
    ...
    while count > 0 do
        ...
        if self.deflateFilled == 0 then -- Start DEFLATE block
            ...
            local header = {  -- 5 bytes long
                bit_and((self.uncompRemain <= DEFLATE_MAX_BLOCK_SIZE and 1 or 0), 0xFF),
                bit_and(bit_rshift(size, 0), 0xFF),
                bit_and(bit_rshift(size, 8), 0xFF),
                bit_and(bit_xor(bit_rshift(size, 0), 0xFF), 0xFF),
                bit_and(bit_xor(bit_rshift(size, 8), 0xFF), 0xFF),
            }
...
function Png:crc32(data, index, len)
    ...
    for i=index,index+len-1 do
        ...
        for j=0,7 do  -- Inefficient bitwise implementation, instead of table-based
            local nbit = bit_and(bit_xor(self.crc, bit_rshift(byte, j)), 1);
            self.crc = bit_xor(bit_rshift(self.crc, 1), bit_and((-nbit), 0xEDB88320));
        end
    end
    self.crc = bit_not(self.crc)
end
